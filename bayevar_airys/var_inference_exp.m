function [negF,wopt,Ropt,epsbf,d0bf] = ...
    var_inference_exp(theta,sig_m,dm,entry,exit,nhat,L,nu,d0_const,num_basis1,num_basis2,X,Y,Xt,Yt,ntrac,sig_t,display)
...
% this function is for convex geometry only
...
% disp('Building basis functions')
d0bf = d0basisfunc(lambda_d0x,lambda_d0y,L_d0x,L_d0y,X(:),Y(:));
[ipbf,ipbf_order2] = intprodbasisfunc(lambda_d0x,lambda_d0y,L_d0x,L_d0y,lambda_Cx,lambda_Cy,L_Cx,L_Cy,entry,exit,nhat);
[id0bf] = intd0basisfunc(lambda_d0x,lambda_d0y,L_d0x,L_d0y,entry,exit,nhat);
...
for i = 1:max_iters          % until convergence of free energy
    ...
    % some stuff for numerical stability
    [U,S,V] = svd(H);
    ...
    backward_steps = 0;
    alpha = alpha_save;
    for k = 0:52        % line search that starts at step length found last time and can decrease or increase
...
plot(yhat(1:n))
hold off
title('Predicted vs supplied measurements')
